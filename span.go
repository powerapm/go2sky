...
func (ds *defaultSpan) Log(time time.Time, ll ...string) {
	var llLen = len(ll)
	if len(ll) == 1 && ds.IsError {
		llLen += 2
	}
	data := make([]*common.KeyStringValuePair, 0, int32(math.Ceil(float64(llLen)/2.0)))

	if len(ll) == 1 && ds.IsError {
		var eventKvp = &common.KeyStringValuePair{}
		eventKvp.Key = "event"
		eventKvp.Value = "error"
		data = append(data, eventKvp)
		var stack = &common.KeyStringValuePair{}
		stack.Key = "stack"
		stack.Value = ll[0]
		data = append(data, stack)
	} else {
